...
Ctf_212 = tf(Vector[[1, 2, 3]; [1, 2]], Vector[[1, 8, 15]; [1, 8, 15]])
Ctf_221 = tf(Vector[[1, 2, 3] [1, 2]], Vector[[1, 8, 15] [1, 8, 15]])
...
Dtf_212 = tf(Vector[[1, 2, 3]; [1, 2]], Vector[[1, 8, 15]; [1, 8, 15]], 0.005)
Dtf_221 = tf(Vector[[1, 2, 3] [1, 2]], Vector[[1, 8, 15] [1, 8, 15]], 0.005)
Dtf_222 = [Dtf_221; Dtf_221]; Dtf_222.Ts = 0.005
Dtf_022 = tf(4*eye(2), 0.005)

@test [Ctf_111 Ctf_221] == tf(Vector[[1,2] [1,2,3] [0,1,2]],
    Vector[[1,5] [1,8,15] [1,8,15]])
@test [Ctf_111; Ctf_212] == tf(Vector[[1,2]; [1,2,3]; [0,1,2]],
    Vector[[1,5]; [1,8,15]; [1,8,15]])
@test append(Ctf_111, Ctf_211) == tf(Vector[[1,2] [0]; [0] [1,2,3]],
    Vector[[1,5] [1]; [1] [1,8,15]]);
@test [Ctf_022 Ctf_222] == tf(Vector[[4] [0] [1,2,3] [0,1,2]; [0] [4] [1,2,3] [0,1,2]],
    Vector[[1] [1] [1,8,15] [1,8,15]; [1] [1] [1,8,15] [1,8,15]])
@test [Ctf_022; Ctf_222] == tf(Vector[[4] [0]; [0] [4]; [1,2,3] [0,1,2]; [1,2,3] [0,1,2]],
    Vector[[1] [1]; [1] [1]; [1,8,15] [1,8,15]; [1,8,15] [1,8,15]])

@test [Dtf_111 Dtf_221] == tf(Vector[[1,2] [1,2,3] [0,1,2]],
    Vector[[1,5] [1,8,15] [1,8,15]], 0.005)
@test [Dtf_111; Dtf_212] == tf(Vector[[1,2]; [1,2,3]; [0,1,2]],
    Vector[[1,5]; [1,8,15]; [1,8,15]], 0.005)
@test append(Dtf_111, Dtf_211) == tf(Vector[[1,2] [0]; [0] [1,2,3]],
    Vector[[1,5] [1]; [1] [1,8,15]], 0.005);
@test [Dtf_022 Dtf_222] == tf(Vector[[4] [0] [1,2,3] [0,1,2]; [0] [4] [1,2,3] [0,1,2]],
    Vector[[1] [1] [1,8,15] [1,8,15]; [1] [1] [1,8,15] [1,8,15]], 0.005)
@test [Dtf_022; Dtf_222] == tf(Vector[[4] [0]; [0] [4]; [1,2,3] [0,1,2]; [1,2,3] [0,1,2]],
    Vector[[1] [1]; [1] [1]; [1,8,15] [1,8,15]; [1,8,15] [1,8,15]], 0.005)

@test series(Ctf_111, Ctf_212) == tf(Vector[[1,4,7,6]; [0,1,4,4]],
