...
@test are(Continuous, a, b, c'*c, r) ≈ [0.5895174372762604 1.8215747248860816; 1.8215747248860823 8.818839806923107]
@test are(Discrete, a, b, c'*c, r) ≈ [240.73344504496302 -131.09928700089387; -131.0992870008943 75.93413176750603]
...
Q = 1.0;
R = 1.0;
X0 = are(Discrete, A, B, Q, R);
X = X0[1]
@test norm(A'X*A - X - (A'X*B)*((B'X*B + R)\(B'X*A)) + Q) < 1e-14
# Test for complex matrices
I2 = Matrix{Float64}(I, 2, 2)
@test are(Discrete, [1.0 im; im 1.0], I2, I2, I2) ≈ [1 + sqrt(2) 0; 0 1 + sqrt(2)]
# And complex scalars
@test are(Discrete, 1.0, 1, 1, 1) ≈ fill((1 + sqrt(5))/2, 1, 1)
@test are(Discrete, 1.0im, 1, 1, 1) ≈ fill((1 + sqrt(5))/2, 1, 1)
@test are(Discrete, 1.0, 1im, 1, 1) ≈ fill((1 + sqrt(5))/2, 1, 1)
