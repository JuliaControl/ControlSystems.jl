...
Base.zero(G::TransferFunction{TE,S}) where {TE,S} = tf(zero(numeric_type(S)), G.timeevol) # can not create a zero of a discrete system from the type alone, the sampletime is not stored.
Base.zero(::Type{TransferFunction{Continuous,S}}) where {S} = tf(zero(numeric_type(S))) 
