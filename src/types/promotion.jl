...
function Base.promote_rule(::Type{StateSpace{SampleT1,T1,MT1}}, ::Type{StateSpace{SampleT2,T2,MT2}}) where {SampleT1,SampleT2,T1,T2,MT1,MT2}
    MT = promote_type(MT1, MT2)
    SampleT = promote_type(SampleT1, SampleT2)
    if isconcretetype(MT) # This sometimes fails and produces a Unionall. This condition can be checked at compile time, checked by f(x,y) = (MT = promote_type(x, y); isconcretetype(MT)); @code_llvm f(Int,Float64)
        StateSpace{SampleT,promote_type(T1, T2),MT}
    else # If fail, fall back to an ordinary matrix
        FT = promote_type(T1, T2)
        StateSpace{SampleT,FT,Matrix{FT}}
    end
end

Base.promote_rule(::Type{StateSpace{SampleT1,T,MT}}, ::Type{HeteroStateSpace{SampleT2,AT,BT,CT,DT}}) where {SampleT1,SampleT2,T,MT,AT,BT,CT,DT} =
    HeteroStateSpace{promote_type(SampleT1, SampleT2),promote_type(MT,AT),promote_type(MT,BT),promote_type(MT,CT),promote_type(MT,DT)}

function Base.promote_rule(::Type{TransferFunction{SampleT1,S1}}, ::Type{StateSpace{SampleT2,T2,MT}}) where {SampleT1,SampleT2,T1,S1<:SisoTf{T1},T2,MT}
    #promote_type(Base.promote_op(/, T1, T1), T2)
    StateSpace{promote_type(SampleT1, SampleT2), promote_type(T1,T2), promote_type(Matrix{T1},MT)}
end
# NOTE: Perhaps should try to keep matrix structure?

function Base.promote_rule(::Type{TransferFunction{SampleT1,S1}}, ::Type{DelayLtiSystem{T2}}) where {SampleT1,T1,S1<:SisoTf{T1},T2}
    DelayLtiSystem{promote_type(T1,T2)}
end
function Base.promote_rule(::Type{StateSpace{SampleT1,T1,MT}}, ::Type{DelayLtiSystem{T2}}) where {SampleT1,T1,MT,T2}
    DelayLtiSystem{promote_type(T1,T2)}
end
...
Base.promote_rule(::Type{TransferFunction{SampleT,S}}, ::Type{T2}) where {SampleT, S<:SisoTf, T2<:Number} =
    TransferFunction{SampleT,promote_type(S, T2)}

# TODO Figure out a better way
function Base.promote_rule(::Type{StateSpace{SampleT, T1, MT}}, ::Type{T2}) where {SampleT, T1, MT, T2<:Number}
...
function Base.promote_rule(::Type{StateSpace{SampleT, T1, MT1}}, ::Type{MT2}) where {SampleT, T1, MT1, MT2<:AbstractMatrix}
    MT = promote_type(MT1, MT2)
    StateSpace{SampleT, eltype(MT), MT}
end
