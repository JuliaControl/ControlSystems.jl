...
function Base.promote_rule(::Type{StateSpace{TimeT1,T1,MT1}}, ::Type{StateSpace{TimeT2,T2,MT2}}) where {TimeT1,TimeT2,T1,T2,MT1,MT2}
    MT = promote_type(MT1, MT2)
    TimeT = promote_type(TimeT1, TimeT2)
    if isconcretetype(MT) # This sometimes fails and produces a Unionall. This condition can be checked at compile time, checked by f(x,y) = (MT = promote_type(x, y); isconcretetype(MT)); @code_llvm f(Int,Float64)
        StateSpace{TimeT,promote_type(T1, T2),MT}
    else # If fail, fall back to an ordinary matrix
        FT = promote_type(T1, T2)
        StateSpace{TimeT,FT,Matrix{FT}}
    end
end

Base.promote_rule(::Type{StateSpace{TimeT1,T,MT}}, ::Type{HeteroStateSpace{TimeT2,AT,BT,CT,DT}}) where {TimeT1,TimeT2,T,MT,AT,BT,CT,DT} =
    HeteroStateSpace{promote_type(TimeT1, TimeT2),promote_type(MT,AT),promote_type(MT,BT),promote_type(MT,CT),promote_type(MT,DT)}

function Base.promote_rule(::Type{TransferFunction{TimeT1,S1}}, ::Type{StateSpace{TimeT2,T2,MT}}) where {TimeT1,TimeT2,T1,S1<:SisoTf{T1},T2,MT}
    #promote_type(Base.promote_op(/, T1, T1), T2)
    StateSpace{promote_type(TimeT1, TimeT2), promote_type(T1,T2), promote_type(Matrix{T1},MT)}
end
# NOTE: Perhaps should try to keep matrix structure?

function Base.promote_rule(::Type{TransferFunction{TimeT1,S1}}, ::Type{DelayLtiSystem{T2}}) where {TimeT1,T1,S1<:SisoTf{T1},T2}
    DelayLtiSystem{promote_type(T1,T2)}
end
function Base.promote_rule(::Type{StateSpace{TimeT1,T1,MT}}, ::Type{DelayLtiSystem{T2}}) where {TimeT1,T1,MT,T2}
    DelayLtiSystem{promote_type(T1,T2)}
end

Base.promote_rule(::Type{TransferFunction{TimeT1,S1}}, ::Type{TransferFunction{TimeT2,S2}}) where {TimeT1, TimeT2, S1, S2} =
    TransferFunction{promote_type(TimeT1,TimeT2),promote_type(S1, S2)}
...
Base.promote_rule(::Type{TransferFunction{TimeT,S}}, ::Type{T2}) where {TimeT, S<:SisoTf, T2<:Number} =
    TransferFunction{TimeT,promote_type(S, T2)}

# TODO Figure out a better way
function Base.promote_rule(::Type{StateSpace{TimeT, T1, MT}}, ::Type{T2}) where {TimeT, T1, MT, T2<:Number}
    NewMT = Base.promote_op(*, MT, T2)
    StateSpace{TimeT, eltype(NewMT), NewMT}
end

Base.promote_rule(::Type{TransferFunction{TimeT, SisoZpk{T1,TR1}}}, ::Type{M2}) where {TimeT, T1, TR1, T2, M2<:AbstractMatrix{T2}} =
    TransferFunction{TimeT, SisoZpk{T1, promote_type(TR1, T2)}}

Base.promote_rule(::Type{TransferFunction{TimeT, SisoRational{T1}}}, ::Type{M2}) where {TimeT, T1, T2, M2<:AbstractMatrix{T2}} =
    TransferFunction{TimeT, SisoRational{promote_type(T1, T2)}}

function Base.promote_rule(::Type{StateSpace{TimeT, T1, MT1}}, ::Type{MT2}) where {TimeT, T1, MT1, MT2<:AbstractMatrix}
    MT = promote_type(MT1, MT2)
    StateSpace{TimeT, eltype(MT), MT}
end
