import Colors
export lsimplot, stepplot, impulseplot, bodeplot, nyquistplot, sigmaplot, marginplot, setPlotScale, gangoffour, gangoffourplot, gangofseven, pzmap, pzmap!, nicholsplot, specificationplot
...
`kwargs` is sent as argument to Plots.plot."""
specificationplot
@recipe function specificationplot(p::Specificationplot;
                                   wint     = (-3,5),
                                   wnum     = 101,
                                   s_labels = ["\$\\sigma (S(j\\omega))\$",
                                               "\$\\sigma (C(j\\omega)S(j\\omega))\$",
                                               "\$\\sigma (T(j\\omega))\$"],
                                   w_labels = ["\$\\gamma \\sigma (W_S(j\\omega)^{-1})\$",
  ...
  for (index, W) in enumerate(weightfunctions)
    ...
    if W isa LTISystem
      if size(W) != (1,1)
        error(ErrorException("We can currently only handle SISO weight funcitions in the visualization"))
      end
...
specificationplot(
  ...
  gamma::Number;
  kwargs...
) where T<:LTISystem = specificationplot(
  sens,
  weight,
  gamma;
  kwargs...
