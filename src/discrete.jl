...
function lsima{T}(sys::StateSpace, t::AbstractVector, r::AbstractVector{T}, control_signal::Function,state,
    ...
    dt = Float64(t[2] - t[1])
    if !Control.iscontinuous(sys) || method == :zoh
        if Control.iscontinuous(sys)
            dsys = Control.c2d(sys, dt, :zoh)[1]
        else
            if sys.Ts != dt
        ...
        dsys, x0map = Control.c2d(sys, dt, :foh)
    end
    n = size(t, 1)
...
function indirect_str(state, y, u,uc, nb,na, lambda,bm1,am,ao,ar=[1],as=[1])
    ...
    a = [1;theta[1:na]]
    b = theta[na+1:end]
    r,s,t = Control.rstd([1],b,a,bm1,am,ao,ar,as)
