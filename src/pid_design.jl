...
"""
    kp, ki = stabregionPID(P, [ω]; kd=0, doplot=false)
...
function stabregionPID(P, ω = _default_freq_vector(P,Val{:bode}()); kd=0, form=:standard)
    ...
    convert_pidparams_from_standard(convert_pidparams_to_standard(kp, ki, kd, :parallel)..., form)[1:2]...
end
...
function loopshapingPI(P, ωp; ϕl=0, rl=0, phasemargin=0, form=:standard, doplot=false)
    ...
    C, convert_pidparams_from_standard(Kp, Ti, Td, form)[1:2]...
end
